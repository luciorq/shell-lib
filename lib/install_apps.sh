...
function install_apps () {
  ...
  _usage="usage: ${0} [--user|--system]";
  \builtin local _arg;
  \builtin local install_type;
  \builtin local apps_length;
  \builtin local app_num_arr;
  \builtin local app_num;
  if [[ ${#} -eq 0 ]]; then
    install_type='--user';
  fi
  for _arg in "${@:-}"; do
...
function __get_gh_latest_tag () {
  ...
  \builtin local tag_arr;
  \builtin local tag_sort_arr;
  \builtin local final_tag;
  ...
  \builtin local required_commands;
  \builtin local cmd;
  required_commands=('grep' 'sed' 'cat' 'curl' 'sort');
  ...
  function __get_app_num_from_app_name () {
    \builtin local input_name;
    \builtin local apps_length;
    \builtin local _app_num;
    \builtin local app_num_arr;
    \builtin local app_num_res;
    \builtin local app_name;
    \builtin local seq_bin;
    seq_bin="$(require 'seq')";
    input_name="${1:-}";
    apps_length="$(get_config apps apps | "${grep_bin}" -c '^name:')";
    \builtin mapfile -t app_num_arr < <(
...
function __install_app_source () {
  ...
  \builtin local tarball_name;
  \builtin local get_url;
  \builtin local dl_path;
  \builtin local build_arr;
  ...
  function __install_app_mamba () {
    \builtin local _usage;
    _usage="Usage: ${0} <--user|--system> <APP_NAME> <BIN_NAME_1> [<BIN_NAME_2> ... <BIN_NAME_N>]";
    ...
    \builtin local _exec_bin;
    \builtin local exec_file;
    # \builtin local python_version;
    install_type="${1:-}";
    app_name="${2:-}";
  ...
  function __install_app_cargo () {
    ...
    \builtin local install_path;
    \builtin local app_name;
    \builtin local app_url;
    \builtin local git_var;
  ...
  function __install_app () {
    ...
    \builtin local _usage="usage: ${0} [--user|--system] <APP_NUM/APP_NAME>";
    \builtin local sed_bin rm_bin;
    # \builtin local cp_bin;
    \builtin local mkdir_bin ln_bin chmod_bin;
    \builtin local _arg;
    \builtin local install_type;
    \builtin local app_num;
    \builtin local num_regex;
    \builtin local link_inst_path;
    \builtin local app_name;
    \builtin local app_version;
    \builtin local app_repo;
    \builtin local app_url;
    \builtin local base_url get_url;
    \builtin local app_type;
    \builtin local tarball_version;
    \builtin local tarball_name;
    \builtin local exec_path_arr;
    \builtin local exec_path;
    \builtin local extra_cmd_arr;
    \builtin local _extra_cmd;
    \builtin local extra_cmd;
    \builtin local app_link;
    \builtin local dl_path lib_path;
    \builtin local install_path;
    \builtin local missing_install;
    \builtin local _link_exec;
    ...
    app_url="$(
      \builtin echo -ne \
        ...
        || \builtin echo -ne '')" \
        | "${sed_bin}" "s|{[ ]*name[ ]*}|${app_name}|g" \
        | "${sed_bin}" "s|{[ ]*version[ ]*}|${tarball_version}|g"
    )";
    if [[ ${app_url} == null ]]; then
      app_url='';
    fi
    ...
    \builtin mapfile -t exec_path_arr < <(
      \builtin echo -ne \
        "$(get_config apps apps "${app_num}" exec_path 2> /dev/null \
          ...
          | "${sed_bin}" "s|{[ ]*name[ ]*}|${app_name}|g" \
          | "${sed_bin}" "s|{[ ]*version[ ]*}|${tarball_version}|g" \
          | "${sed_bin}" "s|{[ ]*repo[ ]*}|${app_repo}|g" \
          | "${sed_bin}" "s|{[ ]*tarball[ ]*}|${tarball_name}|g" \
          | "${sed_bin}" "s|{[ ]*install_path[ ]*}|${install_path}|g" \
          | "${sed_bin}" "s|{[ ]*lib_path[ ]*}|${lib_path}|g"
    )
    if [[ ${exec_path_arr[0]} == null ]]; then
      \builtin declare -a exec_path_arr;
      exec_path_arr=();
      \builtin unset -v exec_path_arr;
      exec_path_arr='';
    fi
    ...
    \builtin local skip_download_step;
    skip_download_step='false';
    \builtin local skip_link_step;
